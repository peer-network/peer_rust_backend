...
async function main() {
    try {
        console.log("\n🚀 Starting token minting to company account...");
        ...
        console.log("\n⏳ Minting tokens to company account...");
        
        try {
            // Use the mintToken instruction with the correct parameters
            const tx = await program.methods
                .mintToken(new anchor.BN(amount))
                .accounts({
                    signer: companyKeypair.publicKey,
                    mint: mintPda,
                    receiver: companyTokenAccount,
